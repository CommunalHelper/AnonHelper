...
namespace Celeste.Mod.Anonhelper {
    public class AnonModule : EverestModule {
            ...
            if (session.HasCloudDash) {
                ...
                switch ((Engine.Scene as Level).Session.CoreMode) {
                    ...
                    case Session.CoreModes.Hot:
                        return Color.DarkRed;
                    default:
                        throw new Exception("invalid coremode");
                }
        ...
        public IEnumerator JellyDelay(Player player) {
            ...
            Glider glider = new(player.Position + (player.Speed / 17) + (player.Speed.X >= 288 ? new Vector2(1 / 2, 1) : new Vector2(0, -1)), true, false);
            DynData<Glider> dyn = new(glider);
